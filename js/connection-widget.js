...
return {
		...
		for (let port in portList) {
			...
			metaLoop: for (let i = 0; i < deviceMeta.length; i++) {
				...
				for (let x = 0; x < deviceMeta[i].VidPids.length; x++) {
					console.log("    x: " + x);
					if (deviceMeta[i].VidPids[x].Vid == portList[port].UsbVid && deviceMeta[i].VidPids[x].Pid == portList[port].UsbPid) {
						matchIndex = i;
						// If this port has any related ports.
						if (portList[port].RelatedNames) {
							let a = Number(port.replace(/com/i, ''));
							let b = Number(portList[port].RelatedNames[0].replace(/com/i, ''));
	...
	portConnected: function (port) {
		...
		// If this method was not called redundantly, update objects and DOM elements.
			...
			consoleLogTabHtml += '<span evt-data="' + port + '" class="console-log-tab">' + port + '</span></li>';
			...
			for(let i = this.consoleLog.openLogs.indexOf(port) + 1; i < this.consoleLog.openLogs.length; i++) {
				...
				publish('/statusbar-widget/add', port, port, "success", that.consoleLog.openLogs[i]);
