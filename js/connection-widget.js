...
define([ 'jquery' ], $ => ({
	...
	initScripts: [
		...
		{   // Punch Press TinyG
			...
			'script': [
				...
				{ Msg: '{y:{am:1,vm:150,fr:150,tn:0,tm:28.346,jm:0.5906,jh:3.937,jd:0.0020,sn:1,sx:0,sv:79,lv:8,lb:3.937,zb:0.188}}', Pause: 1000 },  // Y-Axis Settings jm:1, jh:4
				{ Msg: '{2:{ma:2,sa:1.8,tr:1.0472,mi:8,po:1,pm:3}}', Pause: 500 },
	...
	portConnected(port) {
		...
		const that = this;
		const { watchdogOnPortConnectDelay } = this.consoleLog;
	...
	portFeedstop(port) {
		...
		if (!openPorts)  // If there are no open ports
			return false;
